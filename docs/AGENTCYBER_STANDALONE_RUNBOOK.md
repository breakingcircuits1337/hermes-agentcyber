...
The direct `live-usb/write_usb.sh` and `live-usb/provision.sh` scripts also enforce the same removable-media boundary before destructive work: the operator-supplied path must resolve to a canonical whole-disk `/dev/...` device and Linux must report `removable = 1`. Config directories are packed under top-level `.hermes/`, and prebuilt config tarballs must contain `.hermes/config.yaml`; invalid or incomplete config archives fail closed before provisioning. Root/sudo alone is not sufficient for build/write/provision operations or for non-removable, partition, mapper, symlink-only, unverifiable targets, or invalid provisioned config archives.
